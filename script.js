...
const assetExt={"deer-4":"jpeg","dragon-4":"jpeg","magpie-1":"jpeg","unicorn-1":"jpeg"};
const typeInfo={
...
function finishStage1(){
 ...
 stage2Queue=scores.filter(s=>s.score===max).map(s=>s.key); allStage2Keys=[...stage2Queue];
 document.getElementById('stage1Conclusion').innerHTML=`初步最高分：${stage2Queue.map(k=>typeInfo[k].name).join('、')}<br>接下來將完成 ${stage2Queue.length} 份第二階段問卷。`;
 document.getElementById('scoreTable').innerHTML=`<table class="scoreTable"><tr><th>體質</th><th>初步分數</th></tr>${scores.map(s=>`<tr><td>${typeInfo[s.key].name}</td><td>${s.score} 分</td></tr>`).join('')}</table>`;
